...
} from '../../../core/scrollcomponent/BaseScrollView';
...
export default class ScrollViewer extends BaseScrollView {
  ...
  public componentWillReceiveProps(nextProps: ScrollViewDefaultProps): void {
    if (this.props.distanceFromWindow !== nextProps.distanceFromWindow) {
      if (this._mainDivRef) {
        this._scrollEventNormalizer = new ScrollEventNormalizer(
          this._mainDivRef,
          nextProps.distanceFromWindow
        );
      }
    }
  }
  ...
  public scrollTo(scrollInput: {
    x: number;
    y: number;
    animated: boolean;
  }): void {
    if (scrollInput.animated) {
      this._doAnimatedScroll(
        this.props.horizontal ? scrollInput.x : scrollInput.y
      );
    } else {
      this._setRelevantOffset(
        this.props.horizontal ? scrollInput.x : scrollInput.y
      );
    }
  }
  ...
  public render(): JSX.Element {
    return !this.props.useWindowScroll ? (
      ...
      <div ref={this._setDivRef} style={{ position: 'relative' }}>
        {this.props.children}
      </div>
    );
  }
